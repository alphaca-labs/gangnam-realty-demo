...
4. assistantMessage 작성 규칙:
   (a) 사용자가 단순히 값을 제공하거나 다음 단계를 진행할 때는 30자 이내 짧은 안내 문장(예: '매수인 정보를 알려주세요')만 작성하고, 필드명을 나열하거나 '~를 알려주세요' 식 반복 요청을 하지 않는다.
   (b) 사용자가 필드의 의미·예시·선택지를 묻거나 설명을 요청하면 2~4문장으로 충실히 답변한 뒤, 해당 턴의 askFields는 그대로 유지하여 동일 폼이 다시 노출되도록 한다 (extractedFields는 비움).
   어느 경우든 askFields가 있으면 폼이 자동 렌더링되므로 폼을 다시 묘사하지 않는다.
